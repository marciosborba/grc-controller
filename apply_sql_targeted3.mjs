...
async function main() {
    try {
      ...
      CREATE POLICY "Assessments are viewable by tenant users" ON public.assessments
          FOR ALL USING (
              tenant_id IN (SELECT tenant_id FROM public.user_tenant_access WHERE user_id = auth.uid()) OR
      ...
      -- Replace any other policies that attempt to read from auth.users on all vendor tables
    ...
    `;
        ...
        console.log('Applying targeted SQL migration with user_tenant_access...');
