// Sistema simples de persistência - apenas atualiza static-colors.css
console.log('💾 === SISTEMA SIMPLES DE PERSISTÊNCIA ===');

function simplePersist() {
  ...
  if (!hslParts) {
    console.log('❌ Formato HSL inválido:', currentPrimary);
    return;
  }
  ...
  // Gerar apenas o static-colors.css atualizado
  generateStaticColorsCSS(currentPrimary, primaryHover, primaryGlow);
}

function generateStaticColorsCSS(primary, hover, glow) {
  console.log('📝 Gerando static-colors.css atualizado...');
...
@media (prefers-contrast: high) {
  ...
  console.log('💾 Arquivo static-colors.css baixado!');
  console.log('📁 Substitua src/styles/static-colors.css pelo arquivo baixado');
  console.log('🔄 Recarregue a página (F5) para ver a mudança persistir');
}

// Disponibilizar função globalmente
window.simplePersist = simplePersist;

console.log('\n💡 COMO USAR:');
console.log('1. Aplique uma cor usando os botões ou seletor');
console.log('2. Execute: simplePersist()');
console.log('3. Substitua o arquivo src/styles/static-colors.css');
console.log('4. Recarregue a página (F5)');
console.log('');
console.log('✅ VANTAGEM: Agora só precisa atualizar 1 arquivo!');
console.log('✅ O index.css automaticamente referencia as cores do static-colors.css');
...
setTimeout(() => {
  ...
  if (currentPrimary && !currentPrimary.includes('173 88% 58%') && !currentPrimary.includes('258 90% 66%')) {
    console.log('🎯 Cor personalizada detectada! Execute simplePersist() para salvar.');
  }
}, 1000);