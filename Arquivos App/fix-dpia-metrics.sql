-- ============================================================================
-- CORREÇÃO: Função calculate_privacy_metrics para incluir contagem real de DPIA
-- ============================================================================
...
BEGIN
    ...
    -- DPIA Assessments metrics (CORRIGIDO)
    SELECT jsonb_build_object(
        'total_dpias', COUNT(*),
        'pending_dpias', COUNT(*) FILTER (WHERE status IN ('draft', 'in_progress', 'pending_approval')),
        'completed_dpias', COUNT(*) FILTER (WHERE status IN ('completed', 'approved')),
        'high_risk_dpias', COUNT(*) FILTER (WHERE risk_level IN ('high', 'critical')),
        'requires_anpd_consultation', COUNT(*) FILTER (WHERE anpd_consultation_required = true)
    ) INTO dpia_data
