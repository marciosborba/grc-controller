...
-- Função para obter tema ativo (corrigida)
CREATE OR REPLACE FUNCTION get_active_theme(tenant_uuid UUID DEFAULT NULL)
RETURNS JSON AS $$
DECLARE
    theme_config JSON;
BEGIN
    -- Primeiro, tentar buscar por global_ui_settings
    ...
    -- Se não encontrar, buscar por is_active = true
    IF theme_config IS NULL THEN
        SELECT row_to_json(t.*) INTO theme_config
        FROM global_ui_themes t
        WHERE t.is_active = true 
        AND (t.tenant_id = tenant_uuid OR (tenant_uuid IS NULL AND t.tenant_id IS NULL))
        LIMIT 1;
    END IF;
    
    -- Se ainda não encontrar, usar tema nativo global
    IF theme_config IS NULL THEN
        SELECT row_to_json(t.*) INTO theme_config
        FROM global_ui_themes t
        WHERE t.is_native_theme = true
...
INSERT INTO activity_logs (
    ...
    '{"message": "Fixed apply_theme function to properly sync global_ui_settings and global_ui_themes"}',
    NOW()
