-- =============================================================
-- Migration: Final RLS Stability (v7 - Safe get_auth_tenant_id)
-- Period: 2026-03-09
-- =============================================================

-- 1. Ensure functions are STABLE, SECURITY DEFINER and PL/pgSQL
...
-- 2. Simplified Platform Admins Policies
...
CREATE POLICY "platform_admins_read_self" ON platform_admins FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "platform_admins_read_all" ON platform_admins FOR SELECT USING (is_platform_admin(auth.uid()));

-- 3. PROFILES Policies
-- Using the function to break the recursion chain on SELECT
