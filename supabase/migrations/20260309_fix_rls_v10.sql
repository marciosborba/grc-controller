-- =============================================================
-- Migration: Final RLS Stability (v10 - Optimized Termination)
-- Period: 2026-03-09
-- =============================================================

-- 1. Optimized STABLE Functions
...
DECLARE
    v_tenant_id UUID;
BEGIN
    -- We use a LIMIT 1 to be safe
    SELECT tenant_id INTO v_tenant_id FROM profiles WHERE id = auth.uid() LIMIT 1;
    RETURN v_tenant_id;
END;
$$;

-- 2. Simplified Platform Admins (Public Read authenticated to break loops)
ALTER TABLE platform_admins ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "platform_admins_safe_read" ON platform_admins;
DROP POLICY IF EXISTS "platform_admins_read_self" ON platform_admins;

CREATE POLICY "platform_admins_safe_read" ON platform_admins FOR SELECT USING (true);

-- 3. PROFILES Policies
DROP POLICY IF EXISTS "Users can view profiles from their tenant" ON profiles;
-- Use a CASE or a very specific OR chain to encourage short-circuiting
