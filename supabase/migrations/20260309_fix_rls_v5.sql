-- =============================================================
-- Migration: Final RLS Stability (v5 - Robust PL/pgSQL)
-- Period: 2026-03-09
-- =============================================================

-- 1. Redefine is_platform_admin(uuid) as PL/pgSQL
...
-- 2. Redefine is_platform_admin() as PL/pgSQL
...
-- 3. Simplified Platform Admins Policies (avoid recursion completely)
-- Platform admins should be able to see themselves and other platform admins
-- without calling the function again in the same POLICY evaluation if possible.
...
CREATE POLICY "Users can view profiles from their tenant"
    ...
    USING (
        ...
        OR is_platform_admin() -- Safe because it's PL/pgSQL and SEC DEFINER
    );
...
CREATE POLICY "Admins can update profiles in their tenant"
    ...
    FOR UPDATE
    USING (
        is_platform_admin() -- Safe because it's PL/pgSQL and SEC DEFINER
        OR (
