...
-- Fixed table names (risks -> risk_registrations)
-- SECURITY DEFINER: Runs with privileges of the creator (postgres/admin), bypassing per-table RLS.
...
BEGIN
  ...
  -- Checking for typical high/critical values
  SELECT count(*) INTO critical_risks_count
  FROM risk_registrations
  WHERE risk_level ILIKE '%critical%' OR risk_level ILIKE '%high%' OR risk_level ILIKE '%alto%';
