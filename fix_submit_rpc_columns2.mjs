...
async function main() {
    try {
        ...
        `);
        const validCols = columns.rows.map(r => r.column_name);
        console.log('Valid action_plans columns:', validCols.join(', '));

        // Find the right column for title
        const titleCol = 'title';
        const statusCol = 'status';

        console.log(`Using columns: title -> ${titleCol}, status -> ${statusCol}`);
    ...
    IF jsonb_array_length(p_generated_plans) > 0 THEN
        ...
        FOR v_plan IN SELECT * FROM jsonb_array_elements(p_generated_plans) LOOP
            INSERT INTO action_plans (
                tenant_id, category_id, ${titleCol}, description, priority, due_date, ${statusCol}, related_entity_type, related_entity_id
            ) VALUES (
