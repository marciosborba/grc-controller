...
async function main() {
    try {
        await pool.query(`
DROP FUNCTION IF EXISTS public.create_vendor_auth_user(text, text, text, text, text);
DROP FUNCTION IF EXISTS public.create_vendor_auth_user(text, text, text, uuid, uuid);
...
GRANT EXECUTE ON FUNCTION public.create_vendor_auth_user(text, text, text, uuid, uuid) TO authenticated;
        `);
        console.log('✅ Duplicate RPCs dropped and single uuid RPC created');
    } catch (err) {
