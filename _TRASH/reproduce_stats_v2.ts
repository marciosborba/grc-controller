...
// Mock localStorage and window BEFORE importing anything else
global.localStorage = {
    getItem: () => null,
    setItem: () => { },
    removeItem: () => { },
...
async function run() {
    const { DEFAULT_ASSESSMENT_QUESTIONS, calculateAssessmentStats } = await import('./src/components/vendor-risk/shared/RiskAssessmentManager');
