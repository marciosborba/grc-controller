-- =====================================================
-- SCRIPT COMPLETO PARA POPULAR TODAS AS ABAS
...
-- =====================================================
-- 3. PLANOS DE AÇÃO ADICIONAIS (se necessário)
-- =====================================================

-- Verificar se há assessments sem planos e criar planos básicos
...
SELECT 
    :'tenant_id',
    a.id,
    'PA-' || UPPER(SUBSTRING(a.codigo FROM 1 FOR 3)) || '-2024-' || LPAD((ROW_NUMBER() OVER())::text, 3, '0'),
    'Plano de Melhoria - ' || a.titulo,
    'Plano de ação para tratar gaps e melhorias identificadas no assessment ' || a.titulo,
    'Melhorar conformidade e reduzir riscos identificados no assessment',
    'media',
    'Melhoria da conformidade e redução de riscos operacionais',
    '{"Melhoria da conformidade", "Redução de riscos", "Otimização de processos", "Aumento da eficiência"}',
    CURRENT_DATE + INTERVAL '30 days',
    CURRENT_DATE + INTERVAL '180 days',
    (SELECT id FROM profiles WHERE tenant_id = :'tenant_id' LIMIT 1),
    50000.00,
    '{"Consultoria especializada", "Ferramentas de automação", "Treinamento da equipe", "Documentação de processos"}',
    'planejado',
    0,
    NOW(),
    NOW(),
...
WHERE a.tenant_id = :'tenant_id'
  AND a.status IN ('em_andamento', 'em_revisao', 'concluido')
  AND NOT EXISTS (
    SELECT 1 FROM assessment_action_plans ap 
    WHERE ap.assessment_id = a.id AND ap.tenant_id = :'tenant_id'
  )
LIMIT 3;
...
SELECT 
    ...
    'Gaps identificados para múltiplos assessments' as gaps,
    'Planos de ação para todos os assessments' as planos,
    'Itens de ação distribuídos com status variados' as itens,
