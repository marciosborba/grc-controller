-- ============================================================================
-- INSERÇÃO DO PROMPT ESPECIALISTA EM GESTÃO DE RISCOS (VERSÃO CORRIGIDA)
-- ============================================================================

-- Verificar se já existe o template antes de inserir
DO $$
BEGIN
    -- Inserir template de prompt especializado em gestão de riscos
    IF NOT EXISTS (SELECT 1 FROM ai_grc_prompt_templates WHERE name = 'ALEX RISK - Especialista em Gestão de Riscos Corporativos') THEN
...
**Vamos juntos construir uma gestão de riscos de classe mundial que protege e impulsiona seu negócio!**',
        ...
        RAISE NOTICE 'Template de prompt ALEX RISK inserido com sucesso!';
    ELSE
        RAISE NOTICE 'Template ALEX RISK já existe no banco de dados.';
    END IF;

    -- Inserir também prompt personalizado para o módulo de riscos
    IF NOT EXISTS (
        SELECT 1 FROM ai_module_prompts 
        WHERE module_name = 'risks' 
        AND prompt_name = 'risk-specialist-assistant'
...
Seja consultivo, prático e focado em resultados de negócio.',
        ...
        RAISE NOTICE 'Prompt do módulo de riscos inserido com sucesso!';
    ELSE
        RAISE NOTICE 'Prompt do módulo de riscos já existe no banco de dados.';
    END IF;
END
$$;