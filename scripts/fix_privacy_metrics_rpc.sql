-- FIX: calculate_privacy_metrics function
-- Adds real counts for DPIA and Data Discovery to ensure Dashboard consistency.
...
BEGIN
    ...
    -- DPIA metrics (Fixed: Now queries dpia_assessments)
    SELECT jsonb_build_object(
        'total_dpias', COUNT(*),
        'pending_dpias', COUNT(*) FILTER (WHERE status IN ('pending_approval', 'in_progress')),
        'approved_dpias', COUNT(*) FILTER (WHERE status = 'approved'),
        'high_risk_dpias', COUNT(*) FILTER (WHERE risk_level = 'high')
    ) INTO dpia_data
    FROM dpia_assessments;

    -- Data Discovery metrics (New: Queries data_discovery_sources)
    SELECT jsonb_build_object(
        'total_sources', COUNT(*),
        'active_sources', COUNT(*) FILTER (WHERE status = 'active')
    ) INTO discovery_data
    ...
    result := jsonb_build_object(
        ...
        'dpia_assessments', dpia_data,
        'data_discovery', discovery_data,
