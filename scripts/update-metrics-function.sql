...
BEGIN
    ...
    SELECT jsonb_build_object(
        ...
        'expiring_soon', COUNT(*) FILTER (WHERE status = 'granted' AND expiry_date IS NOT NULL AND expiry_date <= CURRENT_DATE + INTERVAL '30 days'),
    ...
    SELECT jsonb_build_object(
        ...
        'pending_requests', COUNT(*) FILTER (WHERE status IN ('received', 'under_verification', 'in_progress')),
