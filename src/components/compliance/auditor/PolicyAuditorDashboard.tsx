...
export default function PolicyAuditorDashboard() {
    ...
    const [selectedFramework, setSelectedFramework] = useState<string>('ISO 27001'); // Default/Mock for now
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const [params, fetchedAudits] = await Promise.all([
                policyAuditorService.getActivePolicies(tenantId!),
    ...
    const handleStartAudit = async () => {
        ...
        /* 
           NOTE: In a real scenario, we need a valid GLM API Key. 
           For this demo/implementation, we assume it's set in env or passed via context.
           We'll try to use a placeholder or check if service handles it securely server-side 
           or via key management.
        */
        const apiKey = import.meta.env.VITE_GLM_API_KEY || '';
        if (!apiKey) {
            toast.error('Chave de API IA não configurada (VITE_GLM_API_KEY)');
            return;
        }
        ...
        try {
            toast.info(`Iniciando análise inteligente para: ${policy.title}...`);
            await policyAuditorService.startAudit(
                policy,
                'framework-uuid-placeholder', // In real app, select from frameworks table
                selectedFramework,
                apiKey
            );
            ...
            toast.error('Falha na auditoria automática');
        } finally {
            setAnalyzing(false);
        }
    };
    ...
    const handleViewAudit = async (auditId: string) => {
        setLoading(true);
        try {
            const { audit, matches } = await policyAuditorService.getAuditDetails(auditId);
                            ...
                            <div className="space-y-2">
                                ...
                                <Select value={selectedFramework} onValueChange={setSelectedFramework}>
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value="LGPD">LGPD (Lei 13.709)</SelectItem>
                                        <SelectItem value="GDPR">GDPR</SelectItem>
                                        <SelectItem value="NIST CSF">NIST CSF 2.0</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            <Button
                                ...
                                disabled={analyzing || !selectedPolicyId}
                            >
                                {analyzing ? (
                                            ...
                                            >
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            {match.framework_requirement_code && (
                                                                <Badge variant="secondary" className="font-mono text-xs">
                                                                    req: {match.framework_requirement_code}
                                                                </Badge>
                                                            )}
                                                            <div className={`h-2 w-2 rounded-full ${getStatusColor(match.status)}`} />
                                                        </div>
                                                        <p className="text-sm font-medium">{match.control_description}</p>
                                                        <p className="text-xs text-muted-foreground line-clamp-2">
                                                            Evidência: "{match.detected_evidence}"
                                                        </p>
                                                    </div>
                                                ...
                                                <div className="mt-3 flex items-center gap-4 text-xs text-muted-foreground border-t pt-2">
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        Maturidade: <span className="font-medium text-foreground">{match.maturity_level}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
