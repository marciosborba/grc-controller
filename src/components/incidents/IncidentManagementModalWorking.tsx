...
const IncidentManagementModalWorking: React.FC<IncidentManagementModalProps> = ({
  incident,
  isOpen,
  onClose,
  onSuccess
  ...
  // Determinar tenant_id correto (sabemos que funciona)
  ...
  // Validação simples e funcional (baseada no teste que funcionou)
  const validateForm = (): boolean => {
    console.log('🔍 VALIDAÇÃO INICIADA');
    console.log('📋 FormData:', formData);
    console.log('🏢 Effective Tenant ID:', effectiveTenantId);

    // Validação básica - só o essencial
  ...
  // Função para criar dados seguros para o Supabase (baseada no teste que funcionou)
  ...
  // Submeter formulário (baseado no teste que funcionou)
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <DialogFooter className="flex justify-between items-center pt-6 border-t">
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <span>✅ Validação simplificada ativa</span>
            </div>
...
export default IncidentManagementModalWorking;