...
const IncidentManagementModalFinal: React.FC<IncidentManagementModalProps> = ({
  incident,
  isOpen,
  onClose,
  onSuccess
  ...
  const validateForm = (): boolean => {
    ...
    // CORREÇÃO: Validar UUIDs apenas se forem UUIDs reais (não valores padrão)
    const isRealUUID = (value: string) => {
      return value && 
             value !== 'unspecified' && 
             value !== 'unassigned' && 
             value.trim() !== '' &&
    ...
    if (isRealUUID(formData.reported_by)) {
      console.log('🔍 Validando reported_by UUID:', formData.reported_by);
      if (!isValidUUID(formData.reported_by)) {
        console.log('❌ reported_by inválido:', formData.reported_by);
        newErrors.reported_by = 'ID do usuário reportador inválido';
      } else {
        console.log('✅ reported_by válido');
      }
    } else {
      console.log('ℹ️ reported_by é valor padrão (OK):', formData.reported_by);
    }

    if (isRealUUID(formData.assigned_to)) {
      console.log('🔍 Validando assigned_to UUID:', formData.assigned_to);
      if (!isValidUUID(formData.assigned_to)) {
        console.log('❌ assigned_to inválido:', formData.assigned_to);
        newErrors.assigned_to = 'ID do usuário responsável inválido';
      } else {
        console.log('✅ assigned_to válido');
      }
    } else {
      console.log('ℹ️ assigned_to é valor padrão (OK):', formData.assigned_to);
    }
  ...
  const createSafeUpdateData = (formData: FormData) => {
    ...
    // Validar e adicionar UUIDs apenas se forem UUIDs reais
          ...
          <DialogFooter className="flex justify-between items-center pt-6 border-t">
            <div className="flex items-center gap-2 text-sm text-gray-500">
              {Object.keys(errors).length > 0 && (
                <span className="text-red-500">⚠️ Erros: {Object.keys(errors).join(', ')}</span>
              )}
            </div>
...
export default IncidentManagementModalFinal;