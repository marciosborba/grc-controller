              ...
              <SidebarMenu>
                {group.items.filter(item => hasPermission(item.permissions)).map(item => <SidebarMenuItem key={item.title}>
                      <SidebarMenuButton asChild>
                        <NavLink to={item.url} className={`${getNavCls(isActive(item.url))} flex items-center w-full px-3 py-4 rounded-lg transition-all duration-200 group mb-2`} title={collapsed ? item.title : ''}>
