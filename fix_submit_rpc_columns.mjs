...
async function main() {
    try {
        ...
        `);
        console.log('Valid action_plans columns:', columns.rows.map(r => r.column_name).join(', '));

        console.log('\nFixing RPC...');

        // Update the RPC with the correct columns 'action_title', 'action_description', 'action_status'
        await pool.query(`
...
BEGIN
    ...
    WHERE id = p_id
      ...
      AND status IN ('sent', 'in_progress', 'pending_validation'); -- allow pending_validation re-submission just in case
    ...
    IF jsonb_array_length(p_generated_plans) > 0 THEN
        ...
        FOR v_plan IN SELECT * FROM jsonb_array_elements(p_generated_plans) LOOP
            INSERT INTO action_plans (
                tenant_id, category_id, action_title, action_description, priority, due_date, action_status, related_entity_type, related_entity_id
            ) VALUES (
...
$function$;
        `);
        console.log('✅ RPC submit_vendor_assessment_secure updated with correct column names.');
    } catch (err) {
